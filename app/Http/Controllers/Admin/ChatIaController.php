...
class ChatIaController extends Controller
{
    ...
    public function preguntar(Request $request)
    {
        ...
        // Rotación de múltiples API keys separadas por comas
        $keys = array_filter(array_map('trim', explode(',', $rawKeys)));
        $apiKey = !empty($keys) ? $keys[array_rand($keys)] : $rawKeys;
...
";
        ...
        try {
            ...
            ])->withoutVerifying()->post("https://generativelanguage.googleapis.com/v1/models/gemini-2.5-flash:generateContent?key={$apiKey}", [
                'contents' => [
                    [
                        'parts' => [
